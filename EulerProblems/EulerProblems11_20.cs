...
01 70 54 71 83 51 54 69 16 92 33 48 61 43 52 01 89 19 67 48";
            ...
            int[,] table = new int[lines.Length, nums.Length];

            for (i = 0; i < lines.Length; i++)
            {
                nums = lines[i].Split(' ');
                for (j = 0; j < nums.Length; j++)
                    table[i, j] = Convert.ToInt32(nums[j]);
            }
            ...
            for (i = 0; i < table.GetLength(0); i++)
            {
                ...
                {
                    product = table[i, j];
                    if (i < table.GetLength(0) - 3)
                    {
                        downProd(table, ref i, ref j, ref product, ref maxProduct);
                        if (j < table.GetLength(1) - 4)
                        {
                            rightProd(table, ref i, ref j, ref product, ref maxProduct);
                            downrightProd(table, ref i, ref j, ref product, ref maxProduct);
                        }
                        if (j > 3)
                            downleftProd(table, ref i, ref j, ref product, ref maxProduct);
                    }
                    else if(j < table.GetLength(1) - 4)
                        rightProd(table, ref i, ref j, ref product, ref maxProduct);
                }
            }
        ...
        public void downProd(int[,] table, ref int i, ref int j, ref int product, ref int maxProduct)
        {
        ...
        public void downrightProd(int[,] table, ref int i, ref int j, ref int product, ref int maxProduct)
        {
            for (int k = 1; k < 4; k++)
        ...
        public void downleftProd(int[,] table, ref int i, ref int j, ref int product, ref int maxProduct)
        {
            for (int k = 1; k < 4; k++)
                product *= table[i + k, j - k];
            if (product > maxProduct)
...
53503534226472524250874054075591789781264330331690";
            ...
            {
                num = Convert.ToInt64(numStrings[i].Substring(0, digits + 1));
                sum += num;
            }
