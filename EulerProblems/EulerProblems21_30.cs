...
namespace EulerProblems
{
    class EulerProblems21_30
    {
        ...
        public void problem29() //How many distinct terms are in the sequence generated by a^b for 2 ≤ a ≤ 100 and 2 ≤ b ≤ 100?
        {
            stopWatch.Restart();

            int a, b;
            List<double> results = new List<double>();
            Console.Write("Problem 29: ");

            for (a = 2; a <= 100; a++)
                for (b = 2; b <= 100; b++)
                    results.Add(Math.Pow(a, b));

            Console.Write(results.Distinct().Count());
            stopWatch.Stop();
            Console.WriteLine("  (" + stopWatch.ElapsedMilliseconds + "ms" + ")");
        }

        public void problem30() //Find the sum of all the numbers that can be written as the sum of fifth powers of their digits.
        {
            stopWatch.Restart();

            Console.Write("Problem 30: ");
            ...
            for (i = 2; i <= 354294; i++)
            {
                ...
                while (tempNum != 0)
                {
                    digitSum += (int)Math.Pow(tempNum % 10, 5);
                    tempNum /= 10;
                }
                if (digitSum == i)
                    totalSum += i;
